...
/* router.get("/cinemaList", checkAdmin, function (req,res) {
    res.render("admin/cinema/list");
}); */
...
router.get("/cinemaList", checkAdmin, async function (req,res) {
    const cinemas = await Cinema.find().populate("createdBy", "email");
    console.log(cinemas);
...
router.post("/cinemaUpdate", checkAdmin, cpUpload, async function (req,res) {
    ...
    const cinema = await Cinema.findById(req.body.id);
    ...
    const data = await Cinema.findByIdAndUpdate(req.body.id, { $set: update });
    res.redirect("/admin/cinemaList");
});
...
router.get ("/cinemaDelete/:id", checkAdmin, async function (req,res) {
    ...
    if(cinema) {
        ...
        const data = await Cinema.findByIdAndDelete(req.params.id);
    }
    res.redirect("/admin/cinemaList");
});
...
router.get("/booking/:id", checkAdmin, async function (req, res) {
    ...
    const cinema = await Cinema.findById(booking.movieId.cinemaId);
